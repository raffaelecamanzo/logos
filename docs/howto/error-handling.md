...
| `doctor` reports an installed-but-**unreachable** hook configuration, or worktree hooks seeded as copies carrying a staleness marker | `core.hooksPath` resolves to a `.logos/hooks` directory that does not exist in *this* working tree, or the platform had no usable symlinks so the seed fell back to copies which have since drifted from the primary's scripts. Before [CR-106](../requests/CR-106-git-hooks-never-fire-in-a-linked-worktree.md) this was silent — `init` reported installed, `git config` returned a value, the scripts were on disk, and no hook ever ran | Run any logos command in the worktree to re-seed (the seed re-derives on engine start), or `logos init --hooks` in the primary and re-enter the worktree. Do **not** absolutise `core.hooksPath` — it must stay the relative `.logos/hooks` per [FR-IN-06](../specs/requirements/FR-IN-06.md) AC1; reachability comes from seeding |
