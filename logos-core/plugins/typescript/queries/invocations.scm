; TypeScript/JS HTTP client-call capture (S-343, capability = "invocations",
; FR-WS-08, CR-108). Its patterns are identical to those in
; `tsx/queries/invocations.scm`: `.ts`/`.js` and `.tsx`/`.jsx` are one language
; across two tree-sitter `Language`s (ADR-09), so a rule landed in one and not
; the other leaves half the TypeScript surface uncovered.
;
...
;
; Droppable on disk at `.logos/plugins/typescript/queries/invocations.scm`
; (FR-PL-04, FR-PL-05).
