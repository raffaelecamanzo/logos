...
fn each_landed_language_reports_the_invocations_capability() {
    ...
    // Rust (pre-CR-108), then S-341 (java), S-343 (typescript, tsx), S-345
    // (go), S-347 (ruby). Subsequent stories append their language here.
    //
    ...
    for name in ["rust", "java", "typescript", "tsx", "go", "ruby"] {
        let plugin = reg
            .iter()
            .find(|p| p.name() == name)
            .unwrap_or_else(|| panic!("{name} is compiled in by default"));
