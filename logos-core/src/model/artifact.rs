...
impl ArtifactRelation {
    ...
    pub const fn bridge_namespace(self) -> Option<BridgeNamespace> {
        match self {
            ...
            // The CR-011 contract/artifact relations are not invocation arms; the
            // HTTP/gRPC arms (S-252/S-253) each add their own variant.
            ArtifactRelation::ProtoImport
            | ArtifactRelation::ProtoType
            | ArtifactRelation::GraphqlType
            | ArtifactRelation::SchemaType
...
mod tests {
    ...
    /// No CR-011 contract/artifact relation is a cross-service invocation arm:
    /// each declares neither descriptor. The HTTP/gRPC/broker arms
    /// (S-252/S-253/S-254) flip their own variants to `Some` one at a time; this
    /// baseline pins that the pre-existing contract relations stay non-arms.
    #[test]
    fn no_contract_relation_is_an_invocation_arm() {
        for rel in [
            ArtifactRelation::ProtoImport,
            ArtifactRelation::ProtoType,
            ArtifactRelation::GraphqlType,
    ...
    fn all_lists_every_variant_in_declaration_order() {
        const fn index(rel: ArtifactRelation) -> usize {
            match rel {
                ...
                ArtifactRelation::BrokerPublish => 9,
                ArtifactRelation::BrokerSubscribe => 10,
            }
        }
